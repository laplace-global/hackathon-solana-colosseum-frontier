...
import { processDeposit } from '@/lib/lending/solana-service';
 ...
 *
 * Lock collateral and add it to the user's position.
 */
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { senderAddress, marketId, amount, idempotencyKey } = body;
